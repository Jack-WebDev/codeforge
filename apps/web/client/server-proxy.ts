...
import { ApiClient } from "@codeforge/client";

const { CLIENT_ID, CLIENT_SECRET, API_URL } = process.env as Record<
  string,
  string
>;
...
export function createProxy<T = any>(
  key: ClientFunctions,
  mapper: Mapper<T> = echo
) {
  return async (request: Request) => {
    ...
    const response =
      (await (apiClient[key] as (...args: any[]) => any)(data)) || {};
