...
export class Validator<Schema extends z.ZodTypeAny> {
   ...
   */
  static create<
    Builder extends (zod: typeof z, refs: typeof refinements) => any,
  >(builder: Builder): Validator<ReturnType<Builder>> {
...
function parseIssues(err: z.ZodError): BadRequestErrorIssue[] {
  ...
  const fieldErrors = Object.values(issues.fieldErrors).flatMap(
    (i) => i
  ) as BadRequestErrorIssue[];
